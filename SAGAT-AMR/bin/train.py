...
def do_train(checkpoint=None, direction='amr', split_both_decoder=False, fp16=False):
    ...
    train_loader = instantiate_loader(
        ...
        dep_vocab_file=config['dep_vocab_file']
    )

    # dev_gold_path = ROOT / 'data/tmp/dev-gold.txt'
    # dev_pred_path = ROOT / 'data/tmp/dev-pred.txt'
    dev_gold_path = ROOT / 'data/AMR/amr_2.0/temp/dev-gold.txt'
    dev_pred_path = ROOT / 'data/AMR/amr_2.0/temp/dev-pred.txt'
    ...
    if config['save_checkpoints']:
        ...
        if config['log_wandb']:
            ...
            root_num = len(list(root.iterdir()))
