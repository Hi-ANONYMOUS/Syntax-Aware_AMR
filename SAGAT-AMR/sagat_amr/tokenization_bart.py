...
class AMRBartTokenizer(BartTokenizer):
    ...
    def from_pretrained(cls, pretrained_model_path, pred_min=5, *args, **kwargs):
        ...
        return inst
    # 根据data/vocab/下的三个文件，将其tokens加入到原BART的词汇表中
        ...
        for i, (backr, tokk) in enumerate(zip(backreferences, linearized_nodes)):
            ...
            if tokk.startswith('"') and tokk.endswith('"'):
                ...
                if is_in_enc:
                    ...
                    bpe_toks = self._tok_bpe(tokk, add_space=True)
            # bpe_toks是将单词转为subword的，bpe_toks可能是原单词，也可能是原单词分了好几个subword.(pick-out-03 -> pick, -, out, -03). _tok_bpe()就是分解的函数
    ...
    # 输入sentences是句子列表，这里是利用vocab，ids化句子。（原句子有些单词分成若干个subwords）
    ...
    # ------------------------自己加-----------------------
    def batch_encode_sentences_self(self, sentences, dep_tag2id, dep_tags, input_tokens, subtoken_map, device=torch.device('cpu')):
        batch_sentences = []
        batch_graph = []

        # 因为一个batch有多个sample，循环处理每个sample
        for sample_index, input_token in enumerate(input_tokens):
            ...
            graph = self.create_graph(dep_tag_sample, subtoken_map_sample, dep_tag2id)
            batch_graph.append(graph)


        sentences = [s for s in sentences]
        extra = {'sentences': sentences,"dep_graphs":batch_graph}
        # dep_graphs = {"dep_graphs":batch_graph}
        batch = super().batch_encode_plus(batch_sentences, return_tensors='pt', pad_to_max_length=True)
        batch = {k: v.to(device) for k, v in batch.items()}
        return batch, extra

    def create_graph(self, dep_tag_sample, subtoken_map_sample, dep_tag2id):
        graph = dgl.graph([])
    ...
    # 将输入的graph,先直接线性化，然后对有些单词进行bpe, 返回bpe的线性化序列
...
class PENMANBartTokenizer(AMRBartTokenizer):
    ...
    # 去掉"/"和变量，用<pointer-01>的方式代替，得到节点。
    def _get_nodes_and_backreferences(self, graph):
        graph_ = copy.deepcopy(graph)
        graph_.metadata = {}
        linearized = penman.encode(graph_)
        linearized_nodes = self._tokenize_encoded_graph(linearized)
        # linearized_nodes是按照规则切分后，得到的节点
        if self.use_pointer_tokens:
            remap = {}
