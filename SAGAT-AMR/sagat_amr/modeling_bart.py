...
from spring_amr.modules import MultiHeadGATLayer, TGATLayer


# -------------------全局变量-----------------------
LENGTH_dep_tag2id = 50
DEP_hidden_size = 100
HIDDEN_size = 1024
GAT_num_heads = 4
DEP_dropout_rate = 0.1
GAT_num_iterations = 2
...
class AMRBartEncoder(nn.Module):
    ...
    def __init__(self, config: bart.BartConfig, embed_tokens, backpointer_idx):
        ...
        # ------------------------------自己加------------------------
        self.dep_edge_embeddings = nn.Embedding(LENGTH_dep_tag2id, DEP_hidden_size)
        nn.init.trunc_normal_(self.dep_edge_embeddings.weight.data, std=0.02)

        self.tt_gat_layer = MultiHeadGATLayer(TGATLayer, HIDDEN_size, HIDDEN_size,
                                              DEP_hidden_size, GAT_num_heads, DEP_dropout_rate)
        self.fusion_gate = self.ffnn(input_size=HIDDEN_size * 2,
                                     num_hidden_layers=0,
                                     hidden_size=-1,
    ...
    def forward(
        self, input_ids, embedded=None, attention_mask=None, dep_graphs=None
    ):
        """
        Args:
        ...
        for index, dep_graph in enumerate(dep_graphs):
            x_reshape = self.get_embedding_of_token(x[index], attention_mask[index])
            graph = dep_graph.to(x.device)
            ...
            token_embeddings_dep = x_reshape
            for i in range(GAT_num_iterations):
                token_embeddings_dep = self.tt_gat_layer(graph, token_embeddings_dep, token_embeddings_dep)

            f = torch.sigmoid(self.fusion_gate(torch.cat([x_reshape, token_embeddings_dep], dim=-1)))
            mix_embeddings = f * x_reshape + (1 - f) * token_embeddings_dep
            if index==0:
                result_embeddings = torch.cat((x[index][0].unsqueeze(0), mix_embeddings, x[index][mix_embeddings.size()[0]+1:]), dim=0).unsqueeze(0)
            else:
                result_embeddings = torch.cat((result_embeddings,torch.cat((x[index][0].unsqueeze(0), mix_embeddings, x[index][mix_embeddings.size()[0]+1:]), dim=0).unsqueeze(0)))

        return result_embeddings, encoder_states, all_attentions

    def get_embedding_of_token(self, input_embedding, input_mask):
        each_x_reshape = input_embedding[~input_mask][1:-1]
        return each_x_reshape
...
class AMRBartModel(bart.PretrainedBartModel):
    ...
    def forward(
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(input_ids=input_ids, attention_mask=attention_mask, dep_graphs=dep_graphs)
...
class AMRBartForConditionalGeneration(bart.PretrainedBartModel):
    ...
    def generate(
        ...
        if self.config.is_encoder_decoder:
            ...
            encoder_outputs: tuple = encoder(input_ids, attention_mask=attention_mask, dep_graphs=dep_graphs)
