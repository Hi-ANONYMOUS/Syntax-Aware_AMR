    ...
    def batch_encode_sentences(self, sentences, device=torch.device('cpu')):
        ...
        for sentence in sentences:
            if isinstance(sentence, str):
                tokens = self.tokenize(sentence, add_special_tokens=True)
        extra = {'sentences': sentences}
        batch = super().batch_encode_plus(sentences, return_tensors='pt', pad_to_max_length=True)
        batch = {k: v.to(device) for k, v in batch.items()}
        return batch, extra

    def find_head(self,dict,head_ix):
        if head_ix in dict.keys():
            return [head_ix] + self.find_head(dict,dict[head_ix])
        else:
            return  [head_ix]

    # encode sentences (input_ids, dep_span_mask)
    def batch_encode_sentences_self(self, sentences, input_tokens, span_tags, device=torch.device('cpu')):
        sentences = [s for s in sentences]
        batch_subtoken_span_mask = []
        batch_sentences = []
        batch_segment = []

        for sample_index, input_token in enumerate(input_tokens):
            all_tokens_one_sample = [i for sen_token in input_token for i in sen_token]
            word_to_subword_map = {}
            sub_index = 0
            for token_index, token in enumerate(all_tokens_one_sample):
                sub_token = self.tokenize(token, add_special_tokens=False)
                word_to_subword_map[token_index] = (sub_index, len(sub_token) + sub_index - 1)
                sub_index += len(sub_token)
            sample_span_tags = span_tags[sample_index]


            sample_span_more_tags = []
            for sen_idx, sen_span in enumerate(sample_span_tags):
                sample_span_sentence_tags = []
                tail_head_dict = dict(sen_span)
                for idx, (tail_ix, head_ix) in enumerate(sen_span):
                    link_list = [tail_ix] + self.find_head(tail_head_dict, head_ix)
                    for index in range(1, len(link_list)):
                        sample_span_sentence_tags.append((link_list[0],link_list[index]))
                sample_span_more_tags.append(list(set(sample_span_sentence_tags)))


            sub_token_span = []
            pre_sen_len = 0
            for sen_idx, sen_span in enumerate(sample_span_more_tags):
                sen_offset = pre_sen_len
                pre_sen_len += len(input_token[sen_idx])
                for idx, (head_ix, tail_ix) in enumerate(sen_span):
                    head_start, head_end = word_to_subword_map[sen_offset + head_ix]
                    tail_start, tail_end = word_to_subword_map[sen_offset + tail_ix]
                    if head_start == head_end:
                        if tail_start == tail_end:
                            sub_token_span.append((head_start, tail_end))
                        else:
                            for i in range(tail_start, tail_end):
                                for j in range(1, tail_end-i+1):
                                    sub_token_span.append((i, i+j))
                            for k in range(tail_start, tail_end+1):
                                sub_token_span.append((head_start, k))
                    else:
                        if tail_start == tail_end:

                            for i in range(head_start, head_end):
                                for j in range(1, head_end - i + 1):
                                    sub_token_span.append((i, i + j))

                                for k in range(head_start, head_end + 1):
                                    sub_token_span.append((k, tail_start))
                        else:
                             for i in range(tail_start, tail_end):
                                 for j in range(1, tail_end - i + 1):
                                     sub_token_span.append((i, i + j))
                             ...
                             for m in range(head_start, head_end+1):
                                 for n in range(tail_start, tail_end):
                                    sub_token_span.append((m, n))

            segments = []
            sent_segment_matrix_batch = []
            sent_segment_matrix = np.zeros([sub_index,sub_index])
            begin_token = 0
            for sen_idx, sen_input in enumerate(input_token):
                tokens_split_sent = self.tokenize(" ".join(sen_input), add_special_tokens=False)
                if sen_idx==0:
                    segments += [0] * len(tokens_split_sent)
                if (sen_idx % 2) == 0 and sen_idx!=0:
                    segments += [0] * len(tokens_split_sent)
                if (sen_idx % 2) != 0:
                    segments += [1] * len(tokens_split_sent)
                sent_segment_matrix[begin_token:begin_token + len(tokens_split_sent), begin_token:begin_token + len(tokens_split_sent)] = np.ones([len(tokens_split_sent),len(tokens_split_sent)])
                begin_token += len(tokens_split_sent)
            batch_segment.append(segments)
            sent_segment_matrix_batch.append(sent_segment_matrix)

            tokens_split_by_sample1 = self.tokenize(" ".join(all_tokens_one_sample), add_special_tokens=False)

            batch_sentences.append(" ".join(all_tokens_one_sample))



            sub_token_span = list(set(sub_token_span))
            sub_token_span_mask = np.eye(sub_index)
            for idx, (start_ix, end_ix) in enumerate(sub_token_span):
                # for (start_ix, end_ix) in span_list:
                sub_token_span_mask[start_ix, end_ix] = 1
                sub_token_span_mask[end_ix, start_ix] = 1
            batch_subtoken_span_mask.append(sub_token_span_mask)
        # ==================================================================================================================
        extra = {'sentences': sentences}
        ...
        dep_span_mask = []
        for subtoken_span_mask in batch_subtoken_span_mask:
            input_span_mask_matrix = np.zeros((max_seq_len, max_seq_len))
            input_span_mask_matrix[1:len(subtoken_span_mask[0]) + 1, 1:len(subtoken_span_mask[0]) + 1] = subtoken_span_mask[
                                                                                                  0:, 0:]
            dep_span_mask.append(input_span_mask_matrix)
        batch["dep_span_mask"] = torch.tensor(dep_span_mask)
        batch_segment_tensor = []
        for segment in batch_segment:
            segment = [0] + segment + [0] * (max_seq_len-len(segment)-1)
            batch_segment_tensor.append(segment)
        batch["segments"] = torch.tensor(batch_segment_tensor)

        batch_sent_segment_matrix = []
        for sent_segment_matrix in sent_segment_matrix_batch:
            mask_matrix = np.zeros((max_seq_len, max_seq_len))
            mask_matrix[1:len(sent_segment_matrix[0]) + 1, 1:len(sent_segment_matrix[0]) + 1] = sent_segment_matrix[
                                                                                                  0:, 0:]
            batch_sent_segment_matrix.append(mask_matrix)
        batch["segments_matrix"] = torch.tensor(batch_sent_segment_matrix)
        batch = {k: v.to(device) for k, v in batch.items()}
        return batch, extra
