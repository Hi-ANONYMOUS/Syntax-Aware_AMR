...
class AMRBartEncoder(nn.Module):
    ...
    def __init__(self, config: bart.BartConfig, embed_tokens, backpointer_idx):
        ...
        self.node_dropping = config.node_dropping
        self.strategy = config.strategy
        self.decoder_attention_heads = config.decoder_attention_heads
        self.dep_head_number = config.dep_head_number
        self.normal_head_number = config.normal_head_number
        self.dep_layer_begin_index = config.dep_layer_begin_index
        self.dep_layer_end_index = config.dep_layer_end_index

        self.segment = nn.Embedding(2,embed_dim)
        if config.data_type == "AMR3.0":
            self.normal_attentions = nn.Parameter(torch.zeros(config.dep_head_number))


    def forward(
        self, input_ids, embedded=None, attention_mask=None, dep_span_mask=None, segments=None, segments_matrix = None
    ):
        """
        Args:
        ...
        # segment embedding， if need.
        # if segments is not None:
        #     segment_embe = self.segment(segments)
        #     x += segment_embe
        #     segments_mask = (1.0 - segments_matrix) * -10000.0
        ...
        # get dep_span_index (1:mask, 0:no mask)
        extended_dep_span_index = dep_span_mask.unsqueeze(1)
        extended_dep_span_index = extended_dep_span_index.to(
            dtype=next(self.parameters()).dtype)  # fp16 compatibility

        # node_dropping
        if self.node_dropping != None:
            bino1 = torch.Tensor(np.random.binomial(1, self.node_dropping, [extended_dep_span_index.size(2), extended_dep_span_index.size(2)])).to(
                next(self.parameters()).device)
            extended_dep_span_index = extended_dep_span_index.mul(bino1)


        extended_dep_span_mask = (1.0 - extended_dep_span_index) * -10000.0



        # get dep_attn_mask accordding to the different strategy
        # hard strategy
        if self.strategy == "HARD":
            dep_attn_mask = torch.zeros_like(extended_dep_span_mask).repeat(1, self.decoder_attention_heads, 1, 1).to(
                dtype=next(self.parameters()).dtype)
            dep_attn_mask[:, 0:self.dep_head_number] = extended_dep_span_mask


        # ----------This part is our own experiments, not mention in the paper.
        # SOFT_1
        # if self.strategy == "SOFT_1":
        #     dep_heads_soft_dep_span_mask = 10000 * self.normal_attentions[:, None, None] * extended_dep_span_index
        #     dep_attn_mask = torch.zeros([extended_dep_span_index.size(0), self.decoder_attention_heads, extended_dep_span_index.size(2), extended_dep_span_index.size(2)]).to(device=extended_dep_span_index.device)
        #     dep_attn_mask[:, 0:self.dep_head_number] = extended_dep_span_mask
        #     dep_span_mask_part = torch.zeros_like(dep_attn_mask).to(
        #         dtype=next(self.parameters()).dtype)  # fp16 compatibility
        #     dep_span_mask_part[:, :self.dep_head_number, :, :] = dep_heads_soft_dep_span_mask
        #     dep_attn_mask = dep_span_mask_part + dep_attn_mask
        #
        #
        # # SOFT_2
        # if self.strategy == "SOFT_2":
        #     dep_heads_soft_dep_span_mask = 100 * self.normal_attentions[:, None, None] * extended_dep_span_index
        #     dep_attn_mask = extended_dep_span_mask.repeat(1, self.decoder_attention_heads, 1, 1).to(
        #         dtype=next(self.parameters()).dtype)  # fp16 compatibility
        #     dep_span_mask_part = torch.zeros_like(dep_attn_mask).to(
        #         dtype=next(self.parameters()).dtype)  # fp16 compatibility
        #     dep_span_mask_part[:, :self.normal_head_number, :, :] = dep_heads_soft_dep_span_mask
        #     dep_attn_mask = dep_span_mask_part + dep_attn_mask
        #
        #
        # # SOFT_3
        # if self.strategy == "SOFT_3":
        #     segments_mask = (1.0 - segments_matrix) * -10000.0
        #     dep_attn_mask = torch.zeros_like(extended_dep_span_mask).repeat(1, self.decoder_attention_heads, 1, 1).to(
        #         dtype=next(self.parameters()).dtype)
        #     dep_attn_mask[:, 0:self.dep_head_number] = extended_dep_span_mask
        #     dep_attn_mask[:, self.dep_head_number:] = segments_mask
        #     dep_span_mask_part = torch.zeros_like(dep_attn_mask).to(
        #         dtype=next(self.parameters()).dtype)  # fp16 compatibility
        #     dep_heads_soft_dep_span_mask = 10000 * self.normal_attentions[:, None, None] * extended_dep_span_index
        #     dep_span_mask_part[:, self.dep_head_number:, :, :] = dep_heads_soft_dep_span_mask
        #     dep_attn_mask = dep_attn_mask + dep_span_mask_part
        ...
        for index, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                attn = None
            # layer settings
            elif self.dep_layer_begin_index <= index < self.dep_layer_end_index:
                x, attn = encoder_layer(x, attention_mask, dep_attn_mask)
            else:
...
class AMRBartModel(bart.PretrainedBartModel):
    ...
    def forward(
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(input_ids=input_ids, attention_mask=attention_mask, dep_span_mask=dep_span_mask, segments=segments, segments_matrix = segments_matrix)
...
class AMRBartForConditionalGeneration(bart.PretrainedBartModel):
    ...
    Examples::
        ...
        outputs = self.compute_logits(
            ...
            decoder_cached_states=decoder_cached_states,
            use_cache=use_cache,
            dep_span_mask=dep_span_mask,
            segments=segments,
            segments_matrix = segments_matrix
        )
    ...
    def generate(
            ...
            dep_span_mask: Optional[torch.LongTensor] = None,
            segments: Optional[torch.LongTensor] = None,
            segments_matrix: Optional[torch.LongTensor] = None,
        ...
        if self.config.is_encoder_decoder:
            ...
            encoder_outputs: tuple = encoder(input_ids, attention_mask=attention_mask, dep_span_mask=dep_span_mask, segments=segments, segments_matrix = segments_matrix)
...
class EncoderLayer(nn.Module):
    def __init__(self, config: bart.BartConfig):
        ...
        # self.selector = nn.ModuleList([AttentionSelector(config) for _ in range(config.encoder_layers)])

    def forward(self, x, encoder_padding_mask, extended_dep_span_mask=None):
        """
        Args:
        ...
        if extended_dep_span_mask is not None:
            x1, attn_weights1 = self.self_attn(
                query=x, key=x, key_padding_mask=encoder_padding_mask, attn_mask=extended_dep_span_mask,
                need_weights=self.output_attentions
            )
            x2, attn_weights2 = self.self_attn(
                query=x, key=x, key_padding_mask=encoder_padding_mask, attn_mask=None,
                need_weights=self.output_attentions
            )
            # simplify this network. remove previous complex module: selector.
            w = F.softmax(self.w)
            x = self.gamma * (w[0] * x1 + w[1] * x2)
            attn_weights = self.gamma * (w[0] * attn_weights1 + w[1] * attn_weights2)
        else:
