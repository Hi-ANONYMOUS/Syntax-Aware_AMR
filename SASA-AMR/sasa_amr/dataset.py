...
class AMRDataset(Dataset):
    ...
    def __init__(
        ...
        self.span_tags = []
        self.input_tokens = []
        input_span_tag_data = read_json(dep_file)
        for index, g in enumerate(graphs):
            ...
            self.span_tags.append(input_span_tag_data[index]['hpsg_list'])
            self.input_tokens.append(input_span_tag_data[index]['tokens'])
    ...
    def __getitem__(self, idx):
        ...
        sample['input_tokens'] = self.input_tokens[idx]
        sample['span_tags'] = self.span_tags[idx]
        # --
    ...
    def collate_fn(self, samples, device=torch.device('cpu')):
        ...
        span_tags = [s['span_tags'] for s in samples]
        x, extra = self.tokenizer.batch_encode_sentences_self(x, input_tokens, span_tags, device=device)
